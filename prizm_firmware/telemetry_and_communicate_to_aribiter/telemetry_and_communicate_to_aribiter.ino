...
// Wall scan: when a path hits a wall, sweep the two forward ultrasonics across
// the obstacle so the arbiter learns its width before replanning. The robot
// pivots from -HALF_SWEEP to +HALF_SWEEP, pausing to take SCAN_SAMPLES readings.
...
void printPoseJSON()
{
  ...
  Serial.print(F(",\"right_ultrasonic_cm\":"));
  Serial.print(cached_right_ultrasonic_cm);

  Serial.print(F(",\"claw_ultrasonic_cm\":"));
  Serial.print(cached_claw_ultrasonic_cm);

  Serial.print(F(",\"front_ir_raw\":"));
  Serial.print(cached_front_ir_raw);

  Serial.print(F(",\"left_ir_raw\":"));
  Serial.print(cached_left_ir_raw);

  Serial.print(F(",\"front_ir_cm\":"));
  Serial.print(cached_front_ir_cm);

  Serial.print(F(",\"left_ir_cm\":"));
  Serial.print(cached_left_ir_cm);
